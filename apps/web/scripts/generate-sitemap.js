...
fs.readFile('./public/tokens-sitemap.xml', 'utf8', async (_err, data) => {
  ...
  try {
    ...
    tokenAddresses.forEach(({ chainName, address }) => {
      const tokenURL = `https://app.rigoblock.com/explore/tokens/${chainName}/${normalizeTokenAddressForCache(address)}`
  ...
  try {
    ...
    for (const chainName of chains) {
      ...
      poolAddresses.forEach((address) => {
        const poolUrl = `https://app.rigoblock.com/explore/pools/${chainName.toLowerCase()}/${normalizeTokenAddressForCache(address)}`
