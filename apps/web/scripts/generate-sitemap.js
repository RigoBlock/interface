...
fs.readFile("./public/tokens-sitemap.xml", "utf8", async (_err, data) => {
  ...
  try {
    ...
    const tokenAddresses = tokensJSON.tokenRankings.TRENDING.tokens.map(
      (token) => {
        return {
          chainName: token.chain.toLowerCase(),
          address: token.address ? token.address : "NATIVE",
        };
      }
    );
    tokenAddresses.forEach(({ chainName, address }) => {
      const tokenURL = `https://app.rigoblock.com/explore/tokens/${chainName}/${normalizeTokenAddressForCache(
        address
      )}`;
  ...
  try {
    ...
    for (const chainName of chains) {
      ...
      const v3PoolAddresses =
        poolsJSON.data.topV3Pools?.map((pool) => pool.address) ?? [];
      const v2PoolAddresses =
        poolsJSON.data.topV2Pairs?.map((pool) => pool.address) ?? [];
      const poolAddresses = v3PoolAddresses.concat(v2PoolAddresses);

      poolAddresses.forEach((address) => {
        const poolUrl = `https://app.rigoblock.com/explore/pools/${chainName.toLowerCase()}/${normalizeTokenAddressForCache(
          address
        )}`;
